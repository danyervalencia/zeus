...
lor:function(btn){var _p=btn.up("#pan");var _pan=_p.up("log_ordenr");var _TE=(fext_btnTE(btn)=="Pdf"?32:33);var _vs=fextpub_sessVar();
	...
	var _param="&xxYear_id="+fext_GV(_p,"year_id")+"&xxFechaini="+_fechi+"&xxFechafin="+_fechf+"&xxDoc_id="+fext_GV(_p,"doc_id")+"&xxDoc_nombre="+_p.down("#doc_id").getRawValue()+"&xxArea_key="+(Ext.isEmpty(_ak)?"":_ak)+"&xxArea_nombre="+_p.down("#area_key").getRawValue()+"&xxMeta_id="+fext_GV(_p,"meta_id")+"&xxSecfunc_codename="+_p.down("#meta_id").getRawValue()+"&xxTarea_key="+(Ext.isEmpty(_tk)?"":_tk)+"&xxTarea_codename="+_p.down("#tarea_key").getRawValue()+"&xxFuefin_id="+(Ext.isEmpty(_ffi)?0:_ffi)+"&xxFuefin_codename="+_p.down("#fuefin_id").getRawValue()+"&xxTiprecur_id="+(Ext.isEmpty(_tri)?0:_tri)+"&xxTiprecur_codename="+_p.down("#tiprecur_id").getRawValue()+"&xxEspedet_id="+(Ext.isEmpty(_edi)?0:_edi)+"&xxEspedet_codename="+_p.down("#espedet_id").getRawValue()+"&xxPers_key="+fext_GV(_p,"pers_key")+"&xxPers_ruc="+fext_GV(_p,"pers_ruc")+"&xxPers_nombre="+fext_GV(_p,"pers_nombre");
