Ext.define("Siace.controller.log.PedEBCS",{extend:"Ext.app.Controller",
lpe:function(btn){var _w=btn.up("window");var _frm=_w.down("form");var _TE=_w.getTE();
	if(_TE==12){fext_CC("log.PedESET").lpeset(_w);}
		...
		for(var _i=0;_i<_r.length;_i++){var _d=_r[_i].data;
			...
			if(_d.peddet_preuni*1 <= 0 && fext_GV(_w,"ped_monto")*1>0){fext_MsgA("Debe indicar el Precio Unitario de item:("+(_i*1+1)+")"+_d.bs_nombre);return false;}
			...
			if(_d.peddet_pretot*1>0){
				for(var _j=0;_j<_rec.length;_j++){if(_rec[_j].get("espedet_id")==_r[_i].get("espedet_id")){_found = true;}}
