Ext.define("Siace.controller.pub.BSFE02",{extend:"Ext.app.Controller",
pbsfe:function(poC){poC.setIconCls("icon_Modify_90");poC.setTitle("Modificar Registro de Familia ::.");var _frm=poC.down("form");
				...
				if(_res.data[0]){_mod.set(_res.data[0]);_frm.loadRecord(_mod);var _cboG=poC.down("#bsg_id");var _cboC=poC.down("#bsc_id");
					_cboG.getStore().load({callback:function(r,oper,succ){
						if(r.length>0){_cboG.setValue(_d.bsg_id);
							...
							fextpub_tabgenFilt({obj:poC.down("#tipbsf_id"),tgp:1055,val:_d.tipbsf_id});
