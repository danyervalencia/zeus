Ext.define('Siace.view.comp.NumberSeparator',{override:"Ext.form.field.Number",alias:'widget.comp_numberseparator',useThousandSeparator:false,
toRawNumber:function(value){return String(value).replace(this.decimalSeparator,'.').replace(new RegExp(Ext.util.Format.thousandSeparator,"g"),''); },
getErrors:function(value){
	...
	if(isNaN(value.replace(Ext.util.Format.thousandSeparator,''))){ errors.push(format(me.nanText, value)); }
	num=me.parseValue(value);
	if(me.minValue===0&&num<0){ errors.push(this.negativeText); }
...
valueToRaw:function(value){
	if(!this.useThousandSeparator) return this.callParent(arguments);
	var me=this; var format="000,000";
	for(vari=0;i<me.decimalPrecision;i++){
		if(i==0) format+=".";
		format+="0";
	}
...
getSubmitValue:function(){
	...
	if(!me.submitLocaleSeparator){ value = me.toRawNumber(value); }
	return value;
},
setMinValue:function(value){
	...
	if(me.disableKeyFilter!==true){
		...
		if(me.allowExponential){allowed+=me.decimalSeparator+'e+-';}
		else{
		...
		if(me.autoStripChars){ me.stripCharsRe=new RegExp('[^'+allowed+']','gi'); }
	}
},
